...
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE n.nspname = 'public' AND c.relname = 'center_subscriptions' AND c.relkind = 'v'
  ) THEN
    NULL;
  ELSIF NOT EXISTS (
    SELECT 1 FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE n.nspname = 'public' AND c.relname = 'center_subscriptions'
  ) THEN
    CREATE VIEW public.center_subscriptions AS
      SELECT
        id,
        center_id,
        plan_id AS plan,
        student_limit_override AS student_limit,
        started_at,
        expires_at,
        created_at,
        updated_at
